...
void capture_pawn(vector<Pawn>& player, vector<pair<float,float>>& player_path, vector<Pawn>& opp, vector<pair<float,float>>& opp_path) {
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            ...
            if ((player[i].pathIndex != -1) && (opp[j].pathIndex != -1)) {
                p_x = player_path[player[i].pathIndex].first;
                p_y = player_path[player[i].pathIndex].second;

                o_x = opp_path[opp[j].pathIndex].first;
                o_y = opp_path[opp[j].pathIndex].second;
